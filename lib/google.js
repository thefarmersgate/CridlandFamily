...
function creds() {
  const raw = Buffer.from(process.env.GOOGLE_SA_KEY, "base64").toString("utf8");
...
export async function calendar() {
  ...
  const ids = process.env.GCAL_ID.split(",").map((s) => s.trim()).filter(Boolean);
...
export async function photos(limit = 60) {
  ...
  const at = await token();
  const qs = new URLSearchParams({
    q: `'${process.env.GDRIVE_FOLDER_ID}' in parents and mimeType contains 'image/' and trashed = false`,
