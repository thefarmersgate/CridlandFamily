...
/** Raw JSON:API attributes of a device sub-resource (status, capabilities...). */
export async function deviceResource(id, name) {
  const r = await ringFetch(`/v1/devices/${encodeURIComponent(id)}/${name}`);
